-- Migration 010: RALPH - Autonomous Story Implementation System
-- Vision: ralph-rapid-deploy.md - AI-driven story execution via n8n
...
CREATE TABLE IF NOT EXISTS ralph_stories (
    ...
    ai_model VARCHAR(100) DEFAULT 'claude-sonnet-4-20250514',  -- KEY: Per-story model selection
...
-- Seed project + 5 stories
-- Use INSERT with ON CONFLICT to avoid duplicates on re-runs
...
-- Note: RIVET-001, 002, 003 use Sonnet 4 (complex architectural work)
-- RIVET-004, 005 use Haiku (simple text/config changes)
...
-- Comments
COMMENT ON TABLE ralph_projects IS 'RALPH project configurations with token budgets and Telegram chat IDs';
COMMENT ON TABLE ralph_stories IS 'Story queue with per-story AI model selection for cost optimization';
...
COMMENT ON COLUMN ralph_stories.ai_model IS 'AI model to use: Sonnet 4 for complex, Haiku for simple stories';

...
Extracted from rivet/workflows/sme_router.py - Production-ready, 95%+ accuracy
"""
...
async def route_to_sme(
    ...
    if vendor == "siemens":
        from rivet_pro.core.prompts.sme.siemens import troubleshoot as siemens_sme
        result = await siemens_sme(query, ocr_result)
    elif vendor == "rockwell":
        from rivet_pro.core.prompts.sme.rockwell import troubleshoot as rockwell_sme
        result = await rockwell_sme(query, ocr_result)
    elif vendor == "abb":
        from rivet_pro.core.prompts.sme.abb import troubleshoot as abb_sme
        result = await abb_sme(query, ocr_result)
    elif vendor == "schneider":
        from rivet_pro.core.prompts.sme.schneider import troubleshoot as schneider_sme
        result = await schneider_sme(query, ocr_result)
    elif vendor == "mitsubishi":
        from rivet_pro.core.prompts.sme.mitsubishi import troubleshoot as mitsubishi_sme
        result = await mitsubishi_sme(query, ocr_result)
    elif vendor == "fanuc":
        from rivet_pro.core.prompts.sme.fanuc import troubleshoot as fanuc_sme
        result = await fanuc_sme(query, ocr_result)
    else:
        # Generic SME (no manufacturer-specific knowledge)
        from rivet_pro.core.prompts.sme.generic import troubleshoot as generic_sme

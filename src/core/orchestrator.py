"""
RalphOrchestrator - Base orchestration framework for autonomous coding tasks.

Provides the foundation for multi-step task execution with retry logic,
...
class RalphOrchestrator(ABC):
    """
    Base orchestration framework for autonomous coding tasks.

    Provides:
    - Abstract methods for task execution stages
    - Retry helper with exponential backoff
    - Cost tracking aggregation
    - Task hash computation for caching
    - Timing/logging for each stage

    Usage:
        class MyOrchestrator(RalphOrchestrator):
            async def execute_task(self, task: dict) -> PipelineStageResult:
    ...
    def __init__(
        ...
        logger.info(
            ...
            f"max_cost=${self.settings.max_cost_per_task:.2f}"
        )
    ...
    async def execute_task(
        ...
        Args:
            ...
            task_data: Task specification including title, description, criteria

        Returns:
            PipelineStageResult with execution results
        """
        pass
    ...
    def compute_task_hash(task_data: str) -> str:
        """Compute SHA256 hash for task caching."""

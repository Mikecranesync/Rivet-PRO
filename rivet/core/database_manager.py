...
Architecture:
    - Supports 6 providers: Supabase, Railway, Neon, Atlas, VPS, Local SQLite
    - All use PostgreSQL + pgvector extension (except local SQLite)
...
Configuration:
    ...
    - DATABASE_PROVIDER: Primary provider (supabase, railway, neon, atlas, vps, local)
    ...
    - ATLAS_DB_URL: Atlas connection string
    - VPS_KB_HOST, VPS_KB_PORT, VPS_KB_USER, VPS_KB_PASSWORD, VPS_KB_DATABASE: VPS credentials
    - LOCAL_DB_PATH: Local SQLite database path
"""
...
class DatabaseProvider:
    ...
    def __init__(self, name: str, connection_string: str):
        ...
        Args:
            name: Provider name (supabase, railway, neon, atlas, vps, local)
...
class DatabaseManager:
    ...
    Environment Variables:
        DATABASE_PROVIDER: Primary provider (default: neon)
        DATABASE_FAILOVER_ENABLED: Enable failover (default: true)
        DATABASE_FAILOVER_ORDER: Failover sequence (default: neon,supabase,railway,local)
    ...
    def set_provider(self, provider_name: str):
        ...
        Args:
            provider_name: Provider to use (supabase, railway, neon, atlas, vps, local)

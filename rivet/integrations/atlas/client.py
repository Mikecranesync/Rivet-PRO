...
class AtlasClient:
    ...
    @traceable(name="atlas_create_asset")
    async def create_asset(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Create a new asset in Atlas CMMS.

        Args:
            data: Asset data dictionary with fields:
                - name (required): Asset name
                - description: Detailed description
                - area: Location or area
        ...
        Returns:
            Created asset object with ID

        Raises:
            AtlasValidationError: If required fields missing or invalid
            AtlasAPIError: If creation fails
        """
        return await self._request("POST", "/assets", json_data=data)

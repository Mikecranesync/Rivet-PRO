-- Migration 007: Create Work Orders Table (with trigger fix)
-- Date: 2025-12-31
...
-- Create work order sequence for auto-numbering (if not exists)
DO $$ BEGIN
...
-- Create CORRECT trigger function (assigns return value)
...
CREATE INDEX IF NOT EXISTS idx_work_orders_conversation ON work_orders(conversation_id);
...
DROP TRIGGER IF EXISTS work_order_updated_at ON work_orders;
...
DROP TRIGGER IF EXISTS work_order_equipment_stats ON work_orders;
